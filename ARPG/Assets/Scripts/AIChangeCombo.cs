...
/// <summary>
/// AI变招功能（远距离突进技能如果离玩家近则进行变招）
/// </summary>
public class AIChangeCombo : StateMachineBehaviour
{
    private AICombatSystem _aiCombatSystem;

    [SerializeField] private float detectionTime;           //变招检测时间(归一化后的百分比)
    ...
    private void CheckChangeCombatTime(Animator animator)
    {
        if (_aiCombatSystem == null) 
            return;
        if (_aiCombatSystem.GetCurrentTarget() == null) 
            return;
